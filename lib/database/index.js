...
Database.prototype.drop = function drop(collectionName, relations, cb) {
  ...
  cb(new Error("Sorry, there is no 'drop' method supported, please set TTL on the model"));
...
Database.prototype.find = function find(collectionName, criteria, cb) {
  ...
  cb(new Error('Please use primary key for the criteria, eg. find("i-am-id")'));
};
...
Database.prototype.create = function create(collectionName, data, cb) {
  ...
  // update by wuyanxin
  // add ttl support
  self.connection.set(noSqlKey, parsedData, data.TTL || -1, function(err) {
    if(err) return cb(err);
...
Database.prototype.update = function update(collectionName, criteria, values, cb) {
  cb(new Error("Sorry, there is no 'update' method, please use 'create' to replace the record"));
};
...
Database.prototype.destroy = function destroy(collectionName, criteria, cb) {
    ...
    !Array.isArray(criteria.where[primary]) &&
     ...
     Utils.count( criteria.where ) == 1
      ...
      this.connection.get(recordKey, function(err, record) {
        ...
        this.connection.del(recordKey, function(err) {
          if(err) return cb(err);
          cb(null, record);
        });
      });
  ...
  cb(new Error('Please use primary key for the criteria, eg. destroy("i-am-id")'));
};
